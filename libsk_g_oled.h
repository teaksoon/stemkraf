...
//
// Program: libsk_g_oled.h
//        : LIBSK OLED Library "G" Version
//        :
...
*/
#ifndef LIBSK_G_OLED_H
    #define LIBSK_G_OLED_H
...
// Pixel Graphics
void sk_oled_showDot(uint8_t _row, uint8_t _col, uint8_t _state );
void sk_oled_showHLine(uint8_t _row, uint8_t _col, uint8_t _len, uint8_t _state);
void sk_oled_showVLine(uint8_t _row, uint8_t _col, uint8_t _len, uint8_t _state);
void sk_oled_showCircle(uint8_t _row, uint8_t _col, uint8_t _radius, uint8_t _state); 
void sk_oled_showLine(uint8_t _row_1, uint8_t _col_1, uint8_t row_2, uint8_t _col_2, uint8_t _state);
void sk_oled_showRadialSpokes(uint8_t _row, uint8_t _col, uint8_t _len, uint8_t _len_idx, uint8_t _step, uint8_t _step_idx, uint8_t _state);
void __skdebug_oled_mem_dump();
// --- end Function Declaration ---

// --- start Global Vars ---
//
uint8_t       _gsk_oled_mem[8][128];        // Graphics Memory
const float   _gsk_pi = 3.14159267;         // Constant PI
...
void _sk_oled_showCharSpace(uint8_t _row, uint8_t _col, uint8_t _len) { 
  ...
  for (int i=0; i < _len; i++) { 
    ...
    _gsk_oled_mem[_row][_col+i] = 0;  // UPDATE OLED MEMORY
  } 
  _sk_oled_twi_stop();   
}

void __skdebug_oled_mem_dump() {
  // Memory Dump for debugging work
  _sk_oled_setRowCol(0,0); _sk_oled_twi_startData();
  for (uint8_t r=0; r < 8; r++) {
    for (uint8_t c=0; c < 128; c++) _sk_oled_twi_write(_gsk_oled_mem[r][c]);      
  } 
  _sk_oled_twi_stop(); 
}
...
void sk_oled_clearScreen() {
  ...
  for (uint8_t r=0; r < 8; r++) {
    for (uint8_t c=0; c < 128; c++) {
      _sk_oled_twi_write(0);      
      _gsk_oled_mem[r][c] = 0; // UPDATE OLED MEMORY
    }
  } 
  _sk_oled_twi_stop();
  _sk_oled_setRowCol(0,0);
...
void sk_oled_showBitmap(uint8_t _row, uint8_t _col, char *_a_byt, uint8_t _len, uint8_t _len_idx) {
  ...
  for (uint8_t i=0; i < _len; i++) {
    ...
    _gsk_oled_mem[_row][_col+i] = _a_byt[i+_len_idx];  // UPDATE OLED MEMORY
  }
  _sk_oled_twi_stop();
}
...
uint8_t byt;  
  ...
  for (int i=0; i < 5; i++) { // EACH FONT is 5 byte in FONT ARRAY
    ...
    _gsk_oled_mem[_row][_col+i] = byt;  // UPDATE OLED MEMORY
  } 
  _sk_oled_twi_stop();   
}
...
char cTmp;    
  ...
  for (uint8_t i=0; i < 5; i++) {   
    ...
    _gsk_oled_mem[_row][_col+(i*2)]   = cTmp;  // UPDATE OLED MEMORY    
    _gsk_oled_mem[_row][_col+(i*2)+1] = cTmp;  // UPDATE OLED MEMORY   
  }
  _sk_oled_twi_stop(); 
  ...
  for (uint8_t i=0; i < 5; i++) {
    ...
    _sk_oled_twi_write(cTmp); // write to OLED
    _sk_oled_twi_write(cTmp); // same thing, to double size
    _gsk_oled_mem[_row+1][_col+(i*2)]   = cTmp;  // UPDATE OLED MEMORY    
    _gsk_oled_mem[_row+1][_col+(i*2)+1] = cTmp;  // UPDATE OLED MEMORY       
  } 
  _sk_oled_twi_stop();
}
...
uint8_t mem_row;
  ...
  mem_row = _row/8;  // Convert from row pixel, OLED row work in memory page 0-7
  // UPDATE OLED MEMORY
  if ( _state == 1) {
    // turn ON
    _gsk_oled_mem[mem_row][_col] = ( _gsk_oled_mem[mem_row][_col] |  (1<<(_row % 8)) ); 
  } else {
    // turn OFF
    _gsk_oled_mem[mem_row][_col] = ( _gsk_oled_mem[mem_row][_col] & ~(1<<(_row % 8)) );
  } 

  _sk_oled_setRowCol(mem_row,_col); 
  _sk_oled_twi_startData();  
  _sk_oled_twi_write(_gsk_oled_mem[mem_row][_col]);
  _sk_oled_twi_stop();    
}

void sk_oled_showHLine(uint8_t _row, uint8_t _col, uint8_t _len, uint8_t _state) {
  for(int i=0; i < _len; i++) {
    sk_oled_showDot(_row,_col+i,_state);
  }
}

void sk_oled_showVLine(uint8_t _row, uint8_t _col, uint8_t _len, uint8_t _state) { 
  for(int i=0; i < _len; i++) {   
    sk_oled_showDot(_row+i,_col,_state);
  }
}
...
int tmp_y = -2*_radius;
  ...
  sk_oled_showDot(_row,_col+_radius,_state);
  sk_oled_showDot(_row,_col-_radius,_state);
  sk_oled_showDot(_row+_radius,_col,_state);
  sk_oled_showDot(_row-_radius,_col,_state);   
  ...
  while(x < y) {
    ...
    x++; tmp_x += 2; f += tmp_x+1;
    sk_oled_showDot(_row+x,_col+y,_state); 
    sk_oled_showDot(_row-x,_col+y,_state);
    sk_oled_showDot(_row+x,_col-y,_state); 
    sk_oled_showDot(_row-x,_col-y,_state);
    sk_oled_showDot(_row+y,_col+x,_state); 
    sk_oled_showDot(_row-y,_col+x,_state);
    sk_oled_showDot(_row+y,_col-x,_state); 
    sk_oled_showDot(_row-y,_col-x,_state);
  }
...
void sk_oled_showLine(uint8_t _row_1, uint8_t _col_1, uint8_t _row_2, uint8_t _col_2, uint8_t _state) { 
int drow,dcol,rs,cs,err,tmp;
uint8_t tmp_pos;
uint8_t tmp_len;

  if (_row_1 ==_row_2) { // Horizontal line, just use existing function
    if (_col_1 < _col_2) {
      tmp_pos = _col_1; tmp_len = _col_2-_col_1;
    } else {
      tmp_pos = _col_2; tmp_len = _col_1-_col_2;
    }
    sk_oled_showHLine(_row_1,tmp_pos,tmp_len+1,_state);
    return;
  }
  if (_col_1==_col_2) { // Vertical line, just use existing function
    if (_row_1 < _row_2) {
      tmp_pos = _row_1; tmp_len = _row_2-_row_1;
    } else {
      tmp_pos = _row_2; tmp_len = _row_1-_row_2;
    }
    sk_oled_showVLine(tmp_pos,_col_1,tmp_len+1,_state); 
    return;
  } 
  ...
  for(;;) {
    sk_oled_showDot(_row_1,_col_1,_state);
...
void sk_oled_showRadialSpokes(uint8_t _row, uint8_t _col, uint8_t _len, uint8_t _len_idx, uint8_t _step, uint8_t _step_idx, uint8_t _state) {
// _step_idx 0 = vertical top, _step_idx 1... clockwise turn
uint8_t row_1, row_2, col_1, col_2;

  row_1  = _row+( _len_idx * cos(_gsk_pi-((2*_gsk_pi)/_step) * _step_idx) );
  col_1  = _col+( _len_idx * sin(_gsk_pi-((2*_gsk_pi)/_step) * _step_idx) );
  row_2  = _row+(     _len * cos(_gsk_pi-((2*_gsk_pi)/_step) * _step_idx) );
  col_2  = _col+(     _len * sin(_gsk_pi-((2*_gsk_pi)/_step) * _step_idx) );

  if (abs(col_2-col_1)==1) col_2 = col_1;  // minor arm adjustment
  if (abs(row_2-row_1)==1) row_2 = row_1;  // minor arm adjustment
    
  sk_oled_showLine(row_1,col_1,row_2,col_2,_state);
}
// --- end "G" version ---
